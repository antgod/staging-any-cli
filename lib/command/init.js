...
var getPackage = function getPackage(behavior, directory, relativePath, code) {
  return function () {
    return behavior(_path2.default.join(process.cwd(), directory, relativePath), code);
  };
};
var parsePackage = function parsePackage(packageMessage) {
  return JSON.parse(packageMessage);
};
var resolvePackageName = function resolvePackageName(directory) {
  return function (packageMessage) {
    return (0, _json.stringify)(_extends({}, packageMessage, { name: directory }), 2);
  };
};
var setPackageName = function setPackageName(behavior, directory, relativePath, callback) {
  return function (packageMessage) {
    return behavior(_path2.default.join(process.cwd(), directory, relativePath), packageMessage, callback);
  };
};
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            _util.promiseChain.apply(undefined, _toConsumableArray(initCommands(parseCommands(gitUrl, directory)))).then(getPackage(_fs2.default.readFileSync, directory, 'package.json', 'utf8')).then(parsePackage).then(resolvePackageName(directory)).then(setPackageName(_fs2.default.writeFile, directory, 'package.json', function (err) {
              if (err) throw err;
